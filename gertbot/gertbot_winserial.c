...
//  wsprintf(comm_name,L"\\\\.\\COM5"); // PATCH!!!!!!!!!!!!!!!!!!
  ...
  // These are for 19200 baud, 10 bits/symbol  is 1/19200 = ~0.5 milli sec / unsigned char
  comm_timing.ReadIntervalTimeout = 2;           // wait after receiving unsigned char
  comm_timing.ReadTotalTimeoutMultiplier = 2;    // Wait per unsigned char requested
...
{ DWORD num_write;
#ifdef GB_SHOW_SER_TX
  for (num_write=0; num_write<bytes; num_write++)
    printf("%02X ",write_buffer[num_write]);
  printf("\n");
#endif
...
// This fails to detect comports which are already opened
// 
int available_com_ports(void)
{  
  TCHAR comm_name[16]; // need only 10 positions
  int com_port_number;
  int com_vector;
